...
//sd-de
  ...
  #ifdef NAMD_AVXTILES
  // 0 - Tiles mode disabled
  // 1 - C1 switching, arithmetic LJ w/ no user specified mix values
  // 2 - C1 switching with LJTable lookup for parameters
  // 3 - Interpolation supporting any switch options (including none)
  static int avxTilesMode;
  // Additional tiles data structures for explicit LJ mixing in force calc
  static float *avx_tiles_eps4_sigma;
  static float *avx_tiles_eps4_sigma_14;
  #endif

  #if defined(NAMD_KNL) || defined(NAMD_AVXTILES)
  // Table interpolation is truncated at 1/r > KNL_TABLE_MAX_R_1
  // KNL_TABLE_MAX_R_1 must be at least 1.f
  // KNL_TABLE_FACTOR = floor(KNL_TABLE_SIZE / KNL_TABLE_MAX_R_1)
  // Sanity checks for preprocessor defines are in ComputeNonbondedUtil.C
  // For Tiles build, increase KNL_TABLE_MAX_R_1 for excluded/modified
  #define KNL_TABLE_SIZE (4693+2)
  #define KNL_TABLE_MAX_R_1 1.15f
  #define KNL_TABLE_FACTOR (4080+2)
  ...
  static float *knl_excl_ener_table;
  static float *knl_excl_grad_table;
  #ifdef NAMD_KNL
