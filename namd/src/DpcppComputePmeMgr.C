...
void createStream(sycl::queue& stream, int deviceID) try {
  // Create a new stream with the given device ID
  // Priorities are not supported for SYCL queues
  const sycl::queue& device_queue = devices_default_queue[deviceID];
  stream = sycl::queue(device_queue.get_context(), device_queue.get_device(),
      dpcppDevice->getExceptionHandler(), sycl::property::queue::in_order());
...
//
// DPCPP implementation of atom storage
//
class DpcppPmeAtomStorage : public PmeAtomStorage {
public:
  DpcppPmeAtomStorage(const bool useIndex) : PmeAtomStorage(useIndex) {}
  ~DpcppPmeAtomStorage() {
...
private:
  ...
  void *alloc_(const int size) try {
    void* p;
    // TODO: Default queue may cause issues
    p = (void *)sycl::malloc_host(size, dpct::get_default_queue());
    return p;
  } catch (sycl::exception const &exc) {
    std::cerr << exc.what() << "Exception caught at file:" << __FILE__
              << ", line:" << __LINE__ << std::endl;
...
//
void PmeAtomFiler::fileAtoms(const int numAtoms, const DpcppAtom* atoms, Lattice &lattice, const PmeGrid &pmeGrid,
...
// Class constructor
//
DpcppComputePmeMgr::DpcppComputePmeMgr() {
  __sdag_init();
  numDevices = 0;
  numTotalPatches = 0;
...
// Class constructor
//
DpcppComputePmeMgr::DpcppComputePmeMgr(CkMigrateMessage *) {
  __sdag_init();
  NAMD_bug("DpcppComputePmeMgr cannot be migrated");
  numDevices = 0;
  numTotalPatches = 0;
  numNodesContributed = 0;
  numDevicesMax = 0;
...
//
DpcppComputePmeMgr::~DpcppComputePmeMgr() {
  // No need to explicitly destroy SYCL queues
}
...
//
void DpcppComputePmeMgr::getHomePencil(PatchID patchID, int& homey, int& homez) {
  ...
  if (patch_y0 < 0 || patch_y1 >= pmeGrid.K2 || patch_z0 < 0 || patch_z1 >= pmeGrid.K3) {
    NAMD_bug("DpcppComputePmeMgr::getHomePencil, patch bounds are outside grid bounds");
  }
  ...
  for (int iz=0;iz < pmeGrid.zBlocks;iz++) {
    for (int iy=0;iy < pmeGrid.yBlocks;iy++) {
      ...
      if (pencil_y1 - pencil_y0 < pmeGrid.order || pencil_z1 - pencil_z0 < pmeGrid.order)
        NAMD_bug("DpcppComputePmeMgr::getHomePencil, pencil size must be >= PMEInterpOrder");
  ...
  if (homey == -1 || homez == -1)
    NAMD_bug("DpcppComputePmeMgr::getHomePencil, home pencil not found");
}

//
// Calculates maximum number of PME pencils
//
void DpcppComputePmeMgr::restrictToMaxPMEPencils() {
  PatchMap *patchMap = PatchMap::Object();
  ...
  while (pid < numPatches) {
    ...
    // Y
    {
      ...
      if (dmin2pi < 0)
        NAMD_bug("DpcppComputePmeMgr::restrictToMaxPMEPencils, Error in dmin2pi");
      ...
      if (dmax2pi < 0)
        NAMD_bug("DpcppComputePmeMgr::restrictToMaxPMEPencils, Error in dmax2pi");
      // If distance is > 1, must decrease the number of y-pencils and try again
      if (dmax2pi > 1) {
        pmeGrid.yBlocks--;
        if (pmeGrid.yBlocks <= 0) break;
    ...
    // Z
    {
      ...
      if (dmin3pi < 0)
        NAMD_bug("DpcppComputePmeMgr::restrictToMaxPMEPencils, Error in dmin3pi");
      ...
      if (dmax3pi < 0)
        NAMD_bug("DpcppComputePmeMgr::restrictToMaxPMEPencils, Error in dmax3pi");
      // If distance is > 1, must decrease the number of z-pencils and try again
      if (dmax3pi > 1) {
        pmeGrid.zBlocks--;
        if (pmeGrid.zBlocks <= 0) break;
  ...
  if (pmeGrid.xBlocks <= 0 || pmeGrid.yBlocks <= 0 || pmeGrid.zBlocks <= 0)
    NAMD_bug("DpcppComputePmeMgr::restrictToMaxPMEPencils, zero PME pencils found");

  if (pmeGrid.xBlocks > pmeGrid.K1 || pmeGrid.yBlocks > pmeGrid.K2|| pmeGrid.zBlocks > pmeGrid.K3)
    NAMD_bug("DpcppComputePmeMgr::restrictToMaxPMEPencils, unable to restrict number of PME pencils");
}

//
// Sets up pencils. May be called multiple times. 
//
void DpcppComputePmeMgr::setupPencils() {
  SimParameters *simParams = Node::Object()->simParameters;
  ...
  int numDevicesTmp = dpcppDevice->getNumDevice();
  ...
  if (pmePencilType == 1) {
    ...
    // (i % numDeviceTot)/dpcppDevice->getNumDevice() = node index
  ...
  if (ijPencilX.size() != numDevices)
    NAMD_bug("DpcppComputePmeMgr::setupPencils, error setting up x-pencils and devices");
...
//
bool DpcppComputePmeMgr::isPmePe(int pe) {
  for (int i=0;i < xPes.size();i++) {
...
//
bool DpcppComputePmeMgr::isPmeNode(int node) {
...
//
bool DpcppComputePmeMgr::isPmeDevice(int deviceID) {
  for (int i=0;i < nodeDeviceList.size();i++) {
    if (dpcppDevice->getDeviceIDbyRank(nodeDeviceList[i].device % dpcppDevice->getNumDevice()) == deviceID) {
      return true;
    }
  }
  return false;
...
//
void DpcppComputePmeMgr::initialize(CkQdMsg *msg) {
  ...
  if (CkMyNode() == 0) {
    ...
    if (pmePencilType == 3) {
      ...
      pmePencilXYZ = CProxy_DpcppPmePencilXYZ::ckNew(xyzOpts);
      pmePencilXYZ[0].initialize(new DpcppPmeXYZInitMsg(pmeGrid));
      ...
      pmePencilXY = CProxy_DpcppPmePencilXY::ckNew(xyOpts);
      pmePencilZ = CProxy_DpcppPmePencilZ::ckNew(zOpts);
      // Send pencil proxies to other nodes
      thisProxy.recvPencils(pmePencilXY, pmePencilZ);
      pmePencilXY.initialize(new DpcppPmeXYInitMsg(pmeGrid, pmePencilXY, pmePencilZ, xyMap, zMap));
      pmePencilZ.initialize(new DpcppPmeXYInitMsg(pmeGrid, pmePencilXY, pmePencilZ, xyMap, zMap));
    } else {
  		// Blocks in all dimensions => 1D FFT
      ...
      zOpts.setStaticInsertion(true);
      pmePencilX = CProxy_DpcppPmePencilX::ckNew(xOpts);
      pmePencilY = CProxy_DpcppPmePencilY::ckNew(yOpts);
      pmePencilZ = CProxy_DpcppPmePencilZ::ckNew(zOpts);
      // Send pencil proxies to other nodes
      thisProxy.recvPencils(pmePencilX, pmePencilY, pmePencilZ);
      pmePencilX.initialize(new DpcppPmeXInitMsg(pmeGrid, pmePencilX, pmePencilY, pmePencilZ, xMap, yMap, zMap));
      pmePencilY.initialize(new DpcppPmeXInitMsg(pmeGrid, pmePencilX, pmePencilY, pmePencilZ, xMap, yMap, zMap));
      pmePencilZ.initialize(new DpcppPmeXInitMsg(pmeGrid, pmePencilX, pmePencilY, pmePencilZ, xMap, yMap, zMap));
    }
  }

}

void DpcppComputePmeMgr::createDevicesAndAtomFiler() {
  if (CkMyNode() != 0)
    NAMD_bug("DpcppComputePmeMgr::createDevicesAndAtomFiler can only be called on root node");
  ...
  for (int i=0;i < numDevicesMax;i++) {
    CProxy_DpcppComputePmeDevice dev = CProxy_DpcppComputePmeDevice::ckNew();
    memcpy(&msg->dev[i], &dev, sizeof(CProxy_DpcppComputePmeDevice));
  }
...
void DpcppComputePmeMgr::recvAtomFiler(CProxy_PmeAtomFiler filer) {
  pmeAtomFiler = filer;
}

void DpcppComputePmeMgr::recvDevices(RecvDeviceMsg* msg) {
  numDevicesMax = msg->numDevicesMax;
  if (numDevices > numDevicesMax)
    NAMD_bug("DpcppComputePmeMgr::recvDevices, numDevices > numDevicesMax");
...
void DpcppComputePmeMgr::recvPencils(CProxy_DpcppPmePencilXYZ xyz) {
  pmePencilXYZ = xyz;
}

void DpcppComputePmeMgr::recvPencils(CProxy_DpcppPmePencilXY xy, CProxy_DpcppPmePencilZ z) {
  pmePencilXY = xy;
  pmePencilZ = z;
}

void DpcppComputePmeMgr::recvPencils(CProxy_DpcppPmePencilX x, CProxy_DpcppPmePencilY y, CProxy_DpcppPmePencilZ z) {
  pmePencilX = x;
  pmePencilY = y;
  pmePencilZ = z;
}
...
//
void DpcppComputePmeMgr::initialize_pencils(CkQdMsg *msg) {
  if (msg != NULL) delete msg;
  ...
  for (int i=0;i < ijPencilX.size();i++) {
    ...
    int deviceID = dpcppDevice->getDeviceIDbyRank(i % numDevicesTmp);
  ...
  if (n > ijPencilX.size()) {
    ...
    try {
      for (int i = 0; i < nextra; i++) {
        extraDevices[i].deviceID = dpcppDevice->getDeviceIDbyRank((i + ijPencilX.size()) % numDevicesTmp);
        createStream(extraDevices[i].stream, extraDevices[i].deviceID);
      }
    } catch (sycl::exception const &exc) {
      std::cerr << exc.what() << "Exception caught at file:" << __FILE__
                << ", line:" << __LINE__ << std::endl;
...
//
void DpcppComputePmeMgr::activate_pencils(CkQdMsg *msg) {
  if (msg != NULL) delete msg;
...
//
int DpcppComputePmeMgr::getNode(int i, int j) {
  if (i < 0 || i >= pmeGrid.yBlocks || j < 0 || j >= pmeGrid.zBlocks)
    NAMD_bug("DpcppComputePmeMgr::getNode, pencil index out of bounds");
...
//
int DpcppComputePmeMgr::getHomeNode(PatchID patchID) {
  int homey, homez;
...
//
int DpcppComputePmeMgr::getDevice(int i, int j) {
  if (i < 0 || i >= pmeGrid.yBlocks || j < 0 || j >= pmeGrid.zBlocks)
    NAMD_bug("DpcppComputePmeMgr::getDevice, pencil index out of bounds");
  int ind = i + j*pmeGrid.yBlocks;
  int device = nodeDeviceList[ind].device;
  if (device == -1)
    NAMD_bug("DpcppComputePmeMgr::getDevice, no device found");
  return device;
}
...
//
int DpcppComputePmeMgr::getDevicePencilY(int i, int j) {
  if (i < 0 || i >= pmeGrid.xBlocks || j < 0 || j >= pmeGrid.zBlocks)
    NAMD_bug("DpcppComputePmeMgr::getDevicePencilY, pencil index out of bounds");
  for (int device=0;device < ijPencilY.size();device++) {
    if (ijPencilY[device].i == i && ijPencilY[device].j == j) return device;
  }
  char str[256];
  sprintf(str, "DpcppComputePmeMgr::getDevicePencilY, no device found at i %d j %d",i,j);
  NAMD_bug(str);
  return -1;
}
...
//
int DpcppComputePmeMgr::getDevicePencilZ(int i, int j) {
  if (i < 0 || i >= pmeGrid.xBlocks || j < 0 || j >= pmeGrid.yBlocks)
    NAMD_bug("DpcppComputePmeMgr::getDevicePencilZ, pencil index out of bounds");
  for (int device=0;device < ijPencilZ.size();device++) {
    if (ijPencilZ[device].i == i && ijPencilZ[device].j == j) return device;
  }
  NAMD_bug("DpcppComputePmeMgr::getDevicePencilZ, no device found");
  return -1;
}
...
//
int DpcppComputePmeMgr::getDeviceIDPencilX(int i, int j) {
...
//
int DpcppComputePmeMgr::getDeviceIDPencilY(int i, int j) {
...
//
int DpcppComputePmeMgr::getDeviceIDPencilZ(int i, int j) {
...
void DpcppComputePmeMgr::recvAtoms(PmeAtomMsg *msg) {
  int device = getDevice(msg->i, msg->j);
  deviceProxy[device].ckLocalBranch()->recvAtoms(msg);
}

DpcppComputePmeDevice::DpcppComputePmeDevice() {
  // __sdag_init();
  numHomePatches = 0;
  forceCapacity = 0;
  force = NULL;
...
DpcppComputePmeDevice::DpcppComputePmeDevice(CkMigrateMessage *m) {
  // __sdag_init();
  numHomePatches = 0;
  forceCapacity = 0;
  force = NULL;
...
DpcppComputePmeDevice::~DpcppComputePmeDevice() try {
  if (streamCreated) {
    // No need to explicitly destroy SYCL queue
  }
  for (int j=0;j < 2;j++)
    for (int i=0;i < pmeAtomStorage[j].size();i++) {
      if (pmeAtomStorageAllocatedHere[i]) delete pmeAtomStorage[j][i];
    }
  if (force != NULL) deallocate_host<DpcppForce>(&force);
...
void DpcppComputePmeDevice::initialize(PmeGrid &pmeGrid_in, int pencilIndexY_in,
                                      int pencilIndexZ_in, int deviceID_in,
                                      int pmePencilType_in,
                                      CProxy_DpcppComputePmeMgr mgrProxy_in,
                                      CProxy_PmeAtomFiler pmeAtomFiler_in) try {
  ...
  for (int j=0;j < 2;j++) {
    ...
    for (int z=zlo;z <= zhi;z++) {
      for (int y=ylo;y <= yhi;y++) {
        ...
        if (y == 0 && z == 0) {
          // Primary pencil
          pmeAtomStorage[j][pp] = new DpcppPmeAtomStorage(pmePencilType != 3);
        } else {
  ...
  createStream(stream, deviceID);
  streamCreated = true;
  pmeRealSpaceCompute = new DpcppPmeRealSpaceCompute(pmeGrid, pencilIndexY, pencilIndexZ,
    deviceID, &stream);
} catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
...
sycl::queue& DpcppComputePmeDevice::getStream() {
  return stream;
}

int DpcppComputePmeDevice::getDeviceID() {
  return deviceID;
}

CProxy_DpcppComputePmeMgr DpcppComputePmeDevice::getMgrProxy() {
  return mgrProxy;
}

void DpcppComputePmeDevice::setPencilProxy(CProxy_DpcppPmePencilXYZ pmePencilXYZ_in) {
  if (pmePencilType != 3)
    NAMD_bug("DpcppComputePmeDevice::setPencilProxy(1), invalid pmePencilType");
  pmePencilXYZ = pmePencilXYZ_in;
}

void DpcppComputePmeDevice::setPencilProxy(CProxy_DpcppPmePencilXY pmePencilXY_in) {
  if (pmePencilType != 2)
    NAMD_bug("DpcppComputePmeDevice::setPencilProxy(2), invalid pmePencilType");
  pmePencilXY = pmePencilXY_in;
}

void DpcppComputePmeDevice::setPencilProxy(CProxy_DpcppPmePencilX pmePencilX_in) {
  if (pmePencilType != 1)
    NAMD_bug("DpcppComputePmeDevice::setPencilProxy(3), invalid pmePencilType");
  pmePencilX = pmePencilX_in;
}

void DpcppComputePmeDevice::activate_pencils() {
  if (pmePencilType == 1) {
...
void DpcppComputePmeDevice::initializePatches(int numHomePatches_in) {
...
void DpcppComputePmeDevice::registerNeighbor() {
...
//
void DpcppComputePmeDevice::recvAtoms(PmeAtomMsg *msg) {
  ...
  if (pmePencilType == 3) {
    ...
    for (int p=0;p < 9;p++) {
      ...
      if (pp == pp0 || numAtoms > 0) {
        if (pmeGrid.yBlocks == 1 && pmeGrid.zBlocks == 1 && (y != 0 || z != 0))
          NAMD_bug("DpcppComputePmeDevice::recvAtoms, problem with atom filing");
    ...
    if (numAtomsCheck + numStrayAtomsPatch < msg->numAtoms)
      NAMD_bug("DpcppComputePmeDevice::recvAtoms, missing atoms");
  }
...
//
void DpcppComputePmeDevice::sendAtomsToNeighbors() {
  for (int z=zlo;z <= zhi;z++) {
    for (int y=ylo;y <= yhi;y++) {
...
void DpcppComputePmeDevice::sendAtomsToNeighbor(int y, int z, int atomIval) {
  // Pencil index  
  int pp = y-ylo + (z-zlo)*yNBlocks;
  ...
  DpcppAtom* atoms = pmeAtomStorage[atomIval][pp]->getAtoms();
  PmeAtomPencilMsg* msgPencil = new (numAtoms, PRIORITY_SIZE) PmeAtomPencilMsg;
  memcpy(msgPencil->atoms, atoms, numAtoms*sizeof(DpcppAtom));
...
void DpcppComputePmeDevice::recvAtomsFromNeighbor(PmeAtomPencilMsg *msg) {
  ...
  if (y < ylo || y > yhi || z < zlo || z > zhi || (y == 0 && z == 0)) {
    NAMD_bug("DpcppComputePmeDevice::recvAtomsFromNeighbor, pencil index outside bounds");
  }
...
void DpcppComputePmeDevice::registerRecvAtomsFromNeighbor() {
  // Primary pencil
  int pp0 = 0-ylo + (0-zlo)*yNBlocks;
...
void DpcppComputePmeDevice::spreadCharge() {
  ...
  DpcppAtom* atoms = pmeAtomStorage[atomI][pp0]->getAtoms();
  // Flip atomI <-> forceI
  std::swap(atomI, forceI);
  // Re-allocate force buffer if needed
  reallocate_host<DpcppForce>(&force, &forceCapacity, numAtoms, 1.5f);
...
//
void DpcppComputePmeDevice::gatherForce() {
  traceUserBracketEvent(DPCPP_PME_SPREADCHARGE_EVENT, beforeWalltime, CmiWallTimer());
  beforeWalltime = CmiWallTimer();
  // (already have the updated lattice)
  pmeRealSpaceCompute->gatherForce(lattice, force);
  // Set callback that will call gatherForceDone() once gatherForce is done
  ((DpcppPmeRealSpaceCompute*)pmeRealSpaceCompute)->gatherForceSetCallback(this);
  // ((DpcppPmeRealSpaceCompute*)pmeRealSpaceCompute)->waitGatherForceDone();
  // gatherForceDone();
}

static inline void gatherForceDoneLoop(int first, int last, void *result, int paraNum, void *param) {
  DpcppComputePmeDevice* c = (DpcppComputePmeDevice *)param;
  c->gatherForceDoneSubset(first, last);
}

void DpcppComputePmeDevice::gatherForceDoneSubset(int first, int last) {
...
void DpcppComputePmeDevice::gatherForceDone() {
  // Primary pencil has the forces

  traceUserBracketEvent(DPCPP_PME_GATHERFORCE_EVENT, beforeWalltime, CmiWallTimer());
...
//
void DpcppComputePmeDevice::sendForcesToNeighbors() {
  // Primary pencil has the forces
  int pp0 = 0-ylo + (0-zlo)*yNBlocks;
  ...
  for (int z=zlo;z <= zhi;z++) {
    for (int y=ylo;y <= yhi;y++) {
      ...
      if (y != 0 || z != 0) {
        ...
        msg->numAtoms = natom;
        memcpy(msg->force, force+atomStart, natom*sizeof(DpcppForce));
...
void DpcppComputePmeDevice::recvForcesFromNeighbor(PmeForcePencilMsg *msg) {
  ...
  if (y < ylo || y > yhi || z < zlo || z > zhi || (y == 0 && z == 0)) {
    NAMD_bug("DpcppComputePmeDevice::recvForcesFromNeighbor, pencil index outside bounds");
  }
  ...
  // neighborForcePencils[pp].force = new DpcppForce[msg->numAtoms];
  // memcpy(neighborForcePencils[pp].force, msg->force, sizeof(DpcppForce)*msg->numAtoms);
  ...
  if (numPatches != homePatchIndexList[forceI][pp].size()) {
    NAMD_bug("DpcppComputePmeDevice::recvForcesFromNeighbor, numPatches incorrect");
  }
...
void DpcppComputePmeDevice::mergeForcesOnPatch(int homePatchIndex) {
  ...
  if (pmePencilType == 3) {
    ...
    if (forceMsg->zeroCopy) {
      ...
      memcpy(forceMsg->force, force+atomStart, numAtoms*sizeof(DpcppForce));
    }
  } else {

    // Zero force array
    // memset(forceMsg->force, 0, numHomeAtoms[forceI][homePatchIndex]*sizeof(DpcppForce));
    memset(forceMsg->force, 0, forceMsg->numAtoms*sizeof(DpcppForce));
    ...
    for (int j=1;j < plList[forceI][homePatchIndex].size();j++) {
      ...
      int atomEnd   = patchPos[pencilPatchIndex];
      int numAtoms = atomEnd-atomStart;
      DpcppForce *dstForce = forceMsg->force;
      // DpcppForce *srcForce = neighborForcePencils[pp].force;
      DpcppForce *srcForce = neighborForcePencilMsgs[pp]->force;
  ...
  // Patch is done => send over to the node that contains the DpcppComputePme compute,
...
void DpcppComputePmeDevice::sendForcesToPatch(PmeForceMsg *forceMsg) {
  // Now we're on the node that has Pe, hence "compute" -pointer is valid
  int pe                  = forceMsg->pe;
  DpcppComputePme *compute = forceMsg->compute;

  // Store message for use in DpcppComputePme, where it'll also be deleted.
...
#include "DpcppComputePmeMgr.def.h"
