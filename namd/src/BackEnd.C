...
void dpcpp_getargs(char**);
// void cuda_initialize();
void cuda_affinity_initialize();
void mic_getargs(char**);
// void mic_initialize();
#ifdef NAMD_AVXTILES
// For "+notiles" commandline option to disable "Tiles" algorithm 
int avxTilesCommandLineDisable = 0;
#endif
...
#endif
#ifdef NAMD_DPCPP
  group.dpcppComputePmeMgr = CProxy_DpcppComputePmeMgr::ckNew();
  group.dpcppComputeMgr = CProxy_DpcppComputeMgr::ckNew();
#endif
...
#ifdef NAMD_DPCPP
  // launch DPC++ runtime threads before affinity is set
  dpcpp_affinity_initialize();
#endif
  ConverseInit(argc, argv, slave_init, 1, 1);  // calls slave_init on others

// idlepoll only matters for non-smp UDP layer
#if (defined(NAMD_CUDA) || defined(NAMD_DPCPP) || defined(NAMD_MIC)) && CMK_NET_VERSION && CMK_SHARED_VARS_UNAVAILABLE && CMK_WHEN_PROCESSOR_IDLE_USLEEP && ! CMK_USE_IBVERBS && ! CMK_USE_TCP
  if ( ! idlepoll ) {
