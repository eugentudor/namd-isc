...
#ifdef NAMD_DPCPP
// Helper functions for creating and getting pointers to DPC++ computes
DpcppComputeNonbonded* getDpcppComputeNonbonded() {
  return DpcppComputeMgr::getDpcppComputeMgr()->getDpcppComputeNonbonded();
}

DpcppComputeNonbonded* createDpcppComputeNonbonded(ComputeID c) {
  return DpcppComputeMgr::getDpcppComputeMgr()->createDpcppComputeNonbonded(c);
}

DpcppComputeBonded* getDpcppComputeBonded() {
  return DpcppComputeMgr::getDpcppComputeMgr()->getDpcppComputeBonded();
}

DpcppComputeBonded* createDpcppComputeBonded(ComputeID c, ComputeMgr* computeMgr) {
  return DpcppComputeMgr::getDpcppComputeMgr()->createDpcppComputeBonded(c, computeMgr);
}
#endif
...
ComputeMgr::createCompute(ComputeID i, ComputeMap *map)
{
    ...
    case computeNonbondedSelfType:
        ...
        if (simParams->useCUDA2) {
          getDpcppComputeNonbonded()->registerComputeSelf(i, map->computeData[i].pids[0].pid);
        } else { } // RAISE Error
...
#elif defined(NAMD_DPCPP)
        if (simParams->useCUDA2) {
          getDpcppComputeNonbonded()->registerComputePair(i, pid2, trans2);
        } else {
        } // RAISE ERROR
#elif defined(NAMD_MIC)
        if (map->directToDevice(i) == 0) {
...
#endif
#ifdef NAMD_DPCPP
    case dpcppComputeNonbondedType:
      c = createDpcppComputeNonbonded(i);
      map->registerCompute(i,c);
      // NOTE: initialize() is called at the end of createComputes(),
      //       after all computes have been created
      //c->initialize();
...
#endif
#ifdef NAMD_DPCPP
    case dpcppComputeBondedType:
      c = createDpcppComputeBonded(i, this);
      map->registerCompute(i,c);
      break;
#endif
    ...
    case computeExclsType:
        ...
        {
          PatchMap::Object()->basePatchIDList(map->computeData[i].node, pids);
          getDpcppComputeBonded()->registerCompute(map->computeData[i].node, map->type(i), pids);
        } else
#endif
  ...
  case computeSelfGromacsPairType:
    ...
    case computeSelfExclsType:
        ...
        if (simParams->bondedCUDA & 16)
        {
          getDpcppComputeBonded()->registerSelfCompute(map->computeData[i].node, map->type(i), map->computeData[i].pids[0].pid);
        } else
#endif
        {
...
#endif
#ifdef NAMD_DPCPP
    case computePmeDpcppType:
        // PatchMap::Object()->basePatchIDList(CkMyPe(),pids);
        // c = new DpcppComputePme(i, pids);
        c = new DpcppComputePme(i, map->computeData[i].pids[0].pid);
        map->registerCompute(i,c);
        c->initialize();
        break;
#endif
...
#ifdef NAMD_DPCPP
        case dpcppComputeBondedType:
            sprintf(user_des, "dpcppComputeBondedType_%d", i);
            break;
#endif
        case computeExclsType:
...
#ifdef NAMD_DPCPP
        case computePmeDpcppType:
            sprintf(user_des, "computePMEDpcppType_%d", i);
            break;
#endif
        case computeEwaldType:
...
#ifdef NAMD_DPCPP
    bool deviceIsMine = ( dpcppDevice->getMasterPe() == CkMyPe() );
    // Place bonded forces on Pe different from non-bonded forces
    int bondedMasterPe = dpcppDevice->getMasterPe();
    // for (int i=0;i < dpcppDevice->getNumPesSharingDevice();i++) {
    //   int pe = dpcppDevice->getPesSharingDevice(i);
    //   if (pe != dpcppDevice->getMasterPe()) {
    //     bondedMasterPe = pe;
    //   }
    // }
    bool deviceIsMineBonded = (CkMyPe() == bondedMasterPe);
...
#if defined(NAMD_CUDA) || defined(NAMD_DPCPP) || defined(NAMD_MIC)
        switch ( map->type(i) )
        {
#ifdef NAMD_CUDA
          // case computePmeCUDAType:
...
#ifdef NAMD_DPCPP
          // case computePmeDpcppType:
          //   // Only create single DpcppComputePme object per PE
          //  if ( map->computeData[i].node != myNode ) continue;
          //  if (ComputePmeCUDACounter > 0) continue;
          //  ComputePmeCUDACounter++;
          //  break;
          case computeNonbondedSelfType:
            if ( ! deviceIsMine ) continue;
            if ( ! dpcppDevice->device_shared_with_pe(map->computeData[i].node) ) continue;
          break;
...
#else // defined(NAMD_CUDA) || defined(NAMD_DPCPP) || defined(NAMD_MIC)
        if ( map->computeData[i].node != myNode ) continue;
#endif
        DebugM(1,"Compute " << i << '\n');
...
//         DebugM(1,"  numPidsAllocated = " << map->computeData[i].numPidsAllocated << '\n');
    ...
    if (simParams->useCUDA2) {
      if (deviceIsMine) {
        getDpcppComputeNonbonded()->assignPatches(this);
        getDpcppComputeNonbonded()->initialize();
      }
    } else {
    } // RAISE ERROR
    ...
    if (simParams->bondedCUDA) {
      if (deviceIsMineBonded) {
        getDpcppComputeBonded()->initialize();
      }
    }
#endif
...
#ifdef NAMD_DPCPP
class DpcppComputeNonbondedMsg : public CMessage_DpcppComputeNonbondedMsg {
public:
  DpcppComputeNonbonded* c;
  int i;
};

void ComputeMgr::sendAssignPatchesOnPe(std::vector<int>& pes, DpcppComputeNonbonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeNonbondedMsg *msg = new DpcppComputeNonbondedMsg;
    msg->c = c;
    thisProxy[pes[i]].recvAssignPatchesOnPe(msg);
  }
}

void ComputeMgr::recvAssignPatchesOnPe(DpcppComputeNonbondedMsg *msg) {
  msg->c->assignPatchesOnPe();
  delete msg;
}

void ComputeMgr::sendSkipPatchesOnPe(std::vector<int>& pes, DpcppComputeNonbonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeNonbondedMsg *msg = new DpcppComputeNonbondedMsg;
    msg->c = c;
    thisProxy[pes[i]].recvSkipPatchesOnPe(msg);
...
void ComputeMgr::recvSkipPatchesOnPe(DpcppComputeNonbondedMsg *msg) {
  msg->c->skipPatchesOnPe();
  delete msg;
}

void ComputeMgr::sendFinishPatchesOnPe(std::vector<int>& pes, DpcppComputeNonbonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeNonbondedMsg *msg = new (PRIORITY_SIZE) DpcppComputeNonbondedMsg;
    SET_PRIORITY(msg, c->sequence(), COMPUTE_PROXY_PRIORITY);
    msg->c = c;
    thisProxy[pes[i]].recvFinishPatchesOnPe(msg);
  }
...
void ComputeMgr::recvFinishPatchesOnPe(DpcppComputeNonbondedMsg *msg) {
  msg->c->finishPatchesOnPe();
  delete msg;
}

void ComputeMgr::sendFinishPatchOnPe(int pe, DpcppComputeNonbonded* c, int i, PatchID patchID) {
  DpcppComputeNonbondedMsg *msg = new (PRIORITY_SIZE) DpcppComputeNonbondedMsg;
  SET_PRIORITY(msg, c->sequence(), COMPUTE_PROXY_PRIORITY + PATCH_PRIORITY(patchID));
  msg->c = c;
  msg->i = i;
...
void ComputeMgr::recvFinishPatchOnPe(DpcppComputeNonbondedMsg *msg) {
  msg->c->finishPatchOnPe(msg->i);
  delete msg;
}

void ComputeMgr::sendOpenBoxesOnPe(std::vector<int>& pes, DpcppComputeNonbonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeNonbondedMsg *msg = new (PRIORITY_SIZE) DpcppComputeNonbondedMsg;
    SET_PRIORITY(msg, c->sequence(), PROXY_DATA_PRIORITY+1); // after bonded
    msg->c = c;
...
void ComputeMgr::recvOpenBoxesOnPe(DpcppComputeNonbondedMsg *msg) {
  msg->c->openBoxesOnPe();
  delete msg;
}

void ComputeMgr::sendFinishReductions(int pe, DpcppComputeNonbonded* c) {
  DpcppComputeNonbondedMsg *msg = new DpcppComputeNonbondedMsg;
  msg->c = c;
  thisProxy[pe].recvFinishReductions(msg);
}

void ComputeMgr::recvFinishReductions(DpcppComputeNonbondedMsg *msg) {
  msg->c->finishReductions();
  delete msg;
}

void ComputeMgr::sendMessageEnqueueWork(int pe, DpcppComputeNonbonded* c) {
  DpcppComputeNonbondedMsg *msg = new DpcppComputeNonbondedMsg;
  msg->c = c;
  thisProxy[pe].recvMessageEnqueueWork(msg);
}

void ComputeMgr::recvMessageEnqueueWork(DpcppComputeNonbondedMsg *msg) {
  msg->c->messageEnqueueWork();
  delete msg;
}

void ComputeMgr::sendLaunchWork(int pe, DpcppComputeNonbonded* c) {
  DpcppComputeNonbondedMsg *msg = new DpcppComputeNonbondedMsg;
  msg->c = c;
  thisProxy[pe].recvLaunchWork(msg);
}

void ComputeMgr::recvLaunchWork(DpcppComputeNonbondedMsg *msg) {
  msg->c->launchWork();
  delete msg;
}

void ComputeMgr::sendUnregisterBoxesOnPe(std::vector<int>& pes, DpcppComputeNonbonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeNonbondedMsg *msg = new DpcppComputeNonbondedMsg;
    msg->c = c;
    thisProxy[pes[i]].recvUnregisterBoxesOnPe(msg);
...
void ComputeMgr::recvUnregisterBoxesOnPe(DpcppComputeNonbondedMsg *msg) {
  msg->c->unregisterBoxesOnPe();
  delete msg;
}

// Bonded offload
class DpcppComputeBondedMsg : public CMessage_DpcppComputeBondedMsg {
public:
  DpcppComputeBonded* c;
  int i;
};

void ComputeMgr::sendAssignPatchesOnPe(std::vector<int>& pes, DpcppComputeBonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeBondedMsg *msg = new DpcppComputeBondedMsg;
    msg->c = c;
    thisProxy[pes[i]].recvAssignPatchesOnPe(msg);
  }
}

void ComputeMgr::recvAssignPatchesOnPe(DpcppComputeBondedMsg *msg) {
  msg->c->assignPatchesOnPe();
  delete msg;
}

void ComputeMgr::sendMessageEnqueueWork(int pe, DpcppComputeBonded* c) {
  DpcppComputeBondedMsg *msg = new DpcppComputeBondedMsg;
  msg->c = c;
  thisProxy[pe].recvMessageEnqueueWork(msg);
}

void ComputeMgr::recvMessageEnqueueWork(DpcppComputeBondedMsg *msg) {
  msg->c->messageEnqueueWork();
  delete msg;
}

void ComputeMgr::sendOpenBoxesOnPe(std::vector<int>& pes, DpcppComputeBonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeBondedMsg *msg = new (PRIORITY_SIZE) DpcppComputeBondedMsg;
    SET_PRIORITY(msg, c->sequence(), PROXY_DATA_PRIORITY);
    msg->c = c;
    thisProxy[pes[i]].recvOpenBoxesOnPe(msg);
  }
...
void ComputeMgr::recvOpenBoxesOnPe(DpcppComputeBondedMsg *msg) {
  msg->c->openBoxesOnPe();
  delete msg;
}

void ComputeMgr::sendLoadTuplesOnPe(std::vector<int>& pes, DpcppComputeBonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeBondedMsg *msg = new DpcppComputeBondedMsg;
    msg->c = c;
    thisProxy[pes[i]].recvLoadTuplesOnPe(msg);
...
void ComputeMgr::recvLoadTuplesOnPe(DpcppComputeBondedMsg *msg) {
  msg->c->loadTuplesOnPe();
  delete msg;
}

void ComputeMgr::sendLaunchWork(int pe, DpcppComputeBonded* c) {
  DpcppComputeBondedMsg *msg = new DpcppComputeBondedMsg;
  msg->c = c;
  thisProxy[pe].recvLaunchWork(msg);
}

void ComputeMgr::recvLaunchWork(DpcppComputeBondedMsg *msg) {
  msg->c->launchWork();
  delete msg;
}

void ComputeMgr::sendFinishPatchesOnPe(std::vector<int>& pes, DpcppComputeBonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeBondedMsg *msg = new (PRIORITY_SIZE) DpcppComputeBondedMsg;
    SET_PRIORITY(msg, c->sequence(), COMPUTE_PROXY_PRIORITY);
    msg->c = c;
...
void ComputeMgr::recvFinishPatchesOnPe(DpcppComputeBondedMsg *msg) {
  msg->c->finishPatchesOnPe();
  delete msg;
}

void ComputeMgr::sendFinishReductions(int pe, DpcppComputeBonded* c) {
  DpcppComputeBondedMsg *msg = new DpcppComputeBondedMsg;
  msg->c = c;
  thisProxy[pe].recvFinishReductions(msg);
}

void ComputeMgr::recvFinishReductions(DpcppComputeBondedMsg *msg) {
  msg->c->finishReductions();
  delete msg;
}

void ComputeMgr::sendUnregisterBoxesOnPe(std::vector<int>& pes, DpcppComputeBonded* c) {
  for (int i=0;i < pes.size();i++) {
    DpcppComputeBondedMsg *msg = new DpcppComputeBondedMsg;
    msg->c = c;
    thisProxy[pes[i]].recvUnregisterBoxesOnPe(msg);
...
void ComputeMgr::recvUnregisterBoxesOnPe(DpcppComputeBondedMsg *msg) {
  msg->c->unregisterBoxesOnPe();
  delete msg;
}
