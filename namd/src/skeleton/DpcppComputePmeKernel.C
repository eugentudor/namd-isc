...
void dpcpp_pme_charges_dev(
  ...
  // TODO: Get a sub-group handle here using get_sub_group() on an sycl::nd_item
  // Hint: Use auto or sycl::sub_group as return type
  BSPLINE_DEFS
  ...
  for ( int i_atom=0; i_atom < atoms_this_warp; ++i_atom ) {
    // TODO: Insert sub-group-level synchronization using barrier()
    float aq=AQ;
    int ai=(int)AI;
    int aj=(int)AJ;
    int ak=(int)AK;
...
// TODO: Templated kernel name for dpcpp_pme_forces_dev
// Hint: Refer to dpcpp_pme_charges_dev
...
DPCPP_PME_FORCES_PROTOTYPE {
  ...
  queue.submit([&](sycl::handler &cgh) {
      ...
      // TODO: Create a local memory accessor, bspline_2factors_acc, using bspline2_factors_range
      // Hint: Refer to the converted code by DPCT in dpct_output/ComputePmeCUDAKernel.dp.cpp
...
// TODO: Template instantiations for dpcpp_pme_forces
// Hint: Refer to dpcpp_pme_charges and DpcppComputePmeKernel.h
