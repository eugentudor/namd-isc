...
{    
...
#if defined(NAMD_CUDA) || defined(NAMD_MIC) || defined(NAMD_AVXTILES)
  #ifdef NAMD_AVXTILES
  ...
  if ( doMigration ) {
    int n = numAtoms;
    FullAtom *a_i = atom.begin();
    #if defined(NAMD_CUDA) || defined(NAMD_AVXTILES) || \
  ...
  { 
    ...
    const BigReal ucenter_z = ucenter.z;
    const int n = numAtoms;
    dpcppAtomList.resize(n);
    DpcppAtom *ac = dpcppAtomPtr = dpcppAtomList.begin();
    const FullAtom *a = atom.begin();
    for ( int k=0; k<n; ++k ) {
      int j = a[k].sortOrder;
      ac[k].x = a[j].position.x - ucenter_x;
...
#elif defined(NAMD_DPCPP)
    ProxyDataMsg *nmsg = new (pdMsgPLLen, pdMsgAvgPLLen, pdMsgVLLen, intRadLen,
      lcpoTypeLen, pdMsgPLExtLen, dpcppAtomLen, PRIORITY_SIZE) ProxyDataMsg; // BEGIN LA, END LA
#else
    ProxyDataMsg *nmsg;
#endif
...
#elif defined(NAMD_DPCPP)
    memcpy(nmsg->dpcppAtomList, dpcppAtomPtr, sizeof(DpcppAtom)*dpcppAtomLen);
#endif
...
#ifdef NAMD_DPCPP
     int persist_size = sizeof(envelope) + sizeof(ProxyDataMsg) + sizeof(CompAtom)*(pdMsgPLLen+pdMsgAvgPLLen+pdMsgVLLen) + intRadLen*sizeof(Real) + lcpoTypeLen*sizeof(int) + sizeof(CompAtomExt)*pdMsgPLExtLen + sizeof(DpcppAtom)*dpcppAtomLen + PRIORITY_SIZE/8 + 2048;
#else
