...
#include "common.h"
#include "Vector.h"
#if !defined(__CUDACC__) || defined(NAMD_DPCPP)
#include "ResizeArray.h"
#endif

// Force inline
#if defined(_MSC_VER)
#define NAMD_INLINE __forceinline
#else
#define NAMD_INLINE __inline__ __attribute__((always_inline))
#endif
...
struct CompAtomExt {
  #if defined(NAMD_CUDA) || defined(NAMD_MIC) || defined(NAMD_AVXTILES) || defined(NAMD_DPCPP)
...
struct DpcppForce {
  // pad needed to match sizeof(sycl::float3) which is 16 bytes
  float x, y, z, pad;
};

#if !defined(__CUDACC__) || defined(NAMD_DPCPP)
#ifdef NAMD_DPCPP
typedef ResizeArray<DpcppAtom> DpcppAtomList;
#else
