...
/* Requires a test for goneril system, as cbrt conflicts with other libraries
#ifndef cbrt
...
void Controller::compareChecksums(int step, int forgiving) {
...
#ifdef NAMD_DPCPP
    checksum = reduction->item(REDUCTION_EXCLUSION_CHECKSUM_DPCPP);
    if ( ((int64)checksum) > molecule->numCalcFullExclusions &&
         ( ! simParams->mollyOn || step % slowFreq ) ) {
      if ( forgiving )
        iout << iWARN << "High global DPC++ exclusion count ("
                      << ((int64)checksum) << " vs "
                      << molecule->numCalcFullExclusions << "), possible error!\n"
          << iWARN << "This warning is not unusual during minimization.\n"
          << iWARN << "Decreasing pairlistdist or cutoff that is too close to periodic cell size may avoid this.\n" << endi;
        ...
        char errmsg[256];
        sprintf(errmsg, "High global DPC++ exclusion count!  (%lld vs %lld)  System unstable or pairlistdist or cutoff too close to periodic cell size.\n",
                (long long)checksum, (long long)molecule->numCalcFullExclusions);
        NAMD_bug(errmsg);
      }
    }
    ...
    if ( ((int64)checksum) && ((int64)checksum) < molecule->numCalcFullExclusions &&
        ! simParams->goGroPair && ! simParams->qmForcesOn) {
      if ( forgiving || ! simParams->fullElectFrequency ) {
        iout << iWARN << "Low global DPC++ exclusion count!  ("
          << ((int64)checksum) << " vs " << molecule->numCalcFullExclusions << ")";
        if ( forgiving ) {
          iout << "\n"
            << iWARN << "This warning is not unusual during minimization.\n"
        ...
        char errmsg[256];
        sprintf(errmsg, "Low global DPC++ exclusion count!  (%lld vs %lld)  System unstable or pairlistdist or cutoff too small.\n",
                (long long)checksum, (long long)molecule->numCalcFullExclusions);
        NAMD_bug(errmsg);
      }
    }
...
void Controller::printTiming(int step) {
	...
	(endWTime - startWTime) / simParams->outputTiming;
      const double nsPerDay =simParams->dt*1e-6*86400/elapsedW;
	...
	CmiPrintf("TIMING: %d  CPU: %g, %g/step  Wall: %g, %g/step, %gns/day"
		  ", %g hours remaining, %f MB of memory in use.\n",
		  step, endCTime, elapsedC, endWTime, elapsedW, nsPerDay,
...
//fepe
  ...
  iout << wallPerStep << " s/step " << daysPerNano << " days/ns " << 1./daysPerNano << " ns/days ";
