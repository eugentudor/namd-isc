  ...
  case 4:
    ...
    if (!CkMyPe()) {
      ...
      if (simParameters->usePMECUDA) {
        // computePmeCUDAMgr/dpcppComputePmeMgr was created in BackEnd.C
...
#else
          CProxy_DpcppComputePmeMgr pme(CkpvAccess(BOCclass_group).dpcppComputePmeMgr);
#endif
...
#ifdef NAMD_DPCPP
    if ( (simParameters->useCUDA2 || simParameters->bondedCUDA) && CkMyRank()==0 ) {
      CProxy_DpcppComputeMgr nb(CkpvAccess(BOCclass_group).dpcppComputeMgr);
      nb.ckLocalBranch()->initialize(new CkQdMsg);
    }
#endif
    break;
