...
// Class constructor
//
DpcppComputeNonbonded::DpcppComputeNonbonded(ComputeID c, int deviceID,
  DpcppNonbondedTables& dpcppNonbondedTables, bool doStreaming) :
Compute(c), deviceID(deviceID), doStreaming(doStreaming), nonbondedKernel(deviceID, dpcppNonbondedTables, doStreaming),
tileListKernel(deviceID, doStreaming)
#ifdef GBIS_CUDA
, GBISKernel(deviceID)
#endif
 {
  ...
  if (simParams->pressureProfileOn) {
    NAMD_die("DpcppComputeNonbonded, pressure profile not supported");
  }
...
// Class destructor
//
DpcppComputeNonbonded::~DpcppComputeNonbonded() {
  dpcppDevice->setDeviceID(deviceID);
  ...
  if (dpcppPatches != NULL) deallocate_host<DpcppPatchRecord>(&dpcppPatches);
...
void DpcppComputeNonbonded::unregisterBox(int i) {
...
void DpcppComputeNonbonded::unregisterBoxesOnPe() {
  if (rankPatches[CkMyRank()].size() == 0)
    NAMD_bug("DpcppComputeNonbonded::unregisterBoxesOnPe, empty rank");
...
//
void DpcppComputeNonbonded::registerComputeSelf(ComputeID cid, PatchID pid) {
  computesChanged = true;
  addPatch(pid);
...
// Only serialized calls allowed
//
void DpcppComputeNonbonded::registerComputePair(ComputeID cid, PatchID* pid, int* trans) {
  computesChanged = true;
  addPatch(pid[0]);
  addPatch(pid[1]);
...
// Add patch
//
void DpcppComputeNonbonded::addPatch(PatchID pid) {
...
// Add compute
//
void DpcppComputeNonbonded::addCompute(ComputeID cid, PatchID pid1, PatchID pid2, Vector offset) {
  ComputeRecord cr;
  cr.cid = cid;
  cr.pid[0] = pid1;
  cr.pid[1] = pid2;
...
//
void DpcppComputeNonbonded::updatePatch(int i) {
  ...
  dpcppPatches[i].numAtoms = numAtoms;
  dpcppPatches[i].numFreeAtoms = numFreeAtoms;
}

int DpcppComputeNonbonded::findPid(PatchID pid) {
  for (int i=0;i < rankPatches[CkMyRank()].size();i++) {
...
void DpcppComputeNonbonded::patchReady(PatchID pid, int doneMigration, int seq) {
  if (doneMigration) {
    int i = findPid(pid);
    if (i == -1)
      NAMD_bug("DpcppComputeNonbonded::patchReady, Patch ID not found");
    updatePatch(i);
  }
  CmiLock(lock);
...
void DpcppComputeNonbonded::gbisP2PatchReady(PatchID pid, int seq) {
  CmiLock(lock);
  Compute::gbisP2PatchReady(pid, seq);
  CmiUnlock(lock);
}

void DpcppComputeNonbonded::gbisP3PatchReady(PatchID pid, int seq) {
  CmiLock(lock);
  Compute::gbisP3PatchReady(pid, seq);
  CmiUnlock(lock);
}

void DpcppComputeNonbonded::assignPatch(int i) {
  PatchMap* patchMap = PatchMap::Object();
  ...
  if (patches[i].patch == NULL) {
    NAMD_bug("DpcppComputeNonbonded::assignPatch, patch not found");
  }
  ...
  if (patches[i].pe != CkMyPe()) {
    NAMD_bug("DpcppComputeNonbonded::assignPatch, patch assigned to incorrect Pe");
  }
#else
  patches[i].pe = CkMyPe();
#endif
...
void DpcppComputeNonbonded::assignPatchesOnPe() {
  if (rankPatches[CkMyRank()].size() == 0)
    NAMD_bug("DpcppComputeNonbonded::assignPatchesOnPe, empty rank");
...
//
void DpcppComputeNonbonded::assignPatches(ComputeMgr* computeMgrIn) {
  ...
  int masterIndex = -1;
  for ( int i=0; i<dpcppDevice->getNumPesSharingDevice(); ++i ) {
    int pe = dpcppDevice->getPesSharingDevice(i);
...
*/
  ...
  for (int i=0;i < dpcppDevice->getDeviceCount();i++) {
    int pe = dpcppDevice->getMasterPeForDeviceID(i);
  ...
  for (int i=0;i < patches.size();i++) {
    ...
    if (pe == -1) {
      ...
      if (proxyPatchPes.size() == 0) {
        ...
        if (pe == -1)
          NAMD_bug("DpcppComputeNonbonded::assignPatches, Unable to choose PE with proxy patch");
    ...
    if (pe < CkNodeFirst(CkMyNode()) || pe >= CkNodeFirst(CkMyNode()) + CkMyNodeSize() )
      NAMD_bug("DpcppComputeNonbonded::assignPatches, Invalid PE for a patch");
...
void DpcppComputeNonbonded::initialize() {
  if (patches.size() > 0) {
    // Allocate DPC++ version of patches
    dpcppDevice->setDeviceID(deviceID);
    allocate_host<DpcppPatchRecord>(&dpcppPatches, patches.size());
   ...
   */
//   maxShmemPerBlock = props.get_local_mem_size();
    maxShmemPerBlock = 1024*1024;
    ...
    queue &deviceDefaultQueue = getDefaultQ();
    myQ =  queue(deviceDefaultQueue.get_context(), deviceDefaultQueue.get_device() ,exception_handler, sycl::property::queue::in_order());
...
//
void DpcppComputeNonbonded::atomUpdate() {
  atomsChangedIn = true;
}
...
//
void DpcppComputeNonbonded::updatePatches() {
  ...
  for (int i=0;i < patches.size();i++) {
    ...
    dpcppPatches[i].atomStart = atomStart;
    int numAtoms = patches[i].numAtoms;
    int numTiles = ((numAtoms-1)/AVXSIZE+1);
    maxTileListLen = std::max(maxTileListLen, numTiles);
    atomStart += numTiles*AVXSIZE;
  }
  atomStorageSize = atomStart;

  if (maxTileListLen >= 65536) {
    NAMD_bug("DpcppComputeNonbonded::updatePatches, maximum number of tiles per tile lists (65536) blown");
  }
}

void DpcppComputeNonbonded::skipPatch(int i) {
  if (CkMyPe() != patches[i].pe)
    NAMD_bug("DpcppComputeNonbonded::skipPatch called on wrong Pe");
...
void DpcppComputeNonbonded::skipPatchesOnPe() {
  if (rankPatches[CkMyRank()].size() == 0)
    NAMD_bug("DpcppComputeNonbonded::skipPatchesOnPe, empty rank");
  for (int i=0;i < rankPatches[CkMyRank()].size();i++) {
...
void DpcppComputeNonbonded::skip() {
  if (CkMyPe() != masterPe)
    NAMD_bug("DpcppComputeNonbonded::skip() called on non masterPe");
...
void DpcppComputeNonbonded::getMaxMovementTolerance(float& maxAtomMovement, float& maxPatchTolerance) {
  if (CkMyPe() != masterPe)
    NAMD_bug("DpcppComputeNonbonded::getMaxMovementTolerance() called on non masterPe");
...
inline void DpcppComputeNonbonded::updateVdwTypesExclLoop(int first, int last, void *result, int paraNum, void *param) {
  DpcppComputeNonbonded* c = (DpcppComputeNonbonded *)param;
  c->updateVdwTypesExclSubset(first, last);
}

void DpcppComputeNonbonded::updateVdwTypesExclSubset(int first, int last) {
...
//
void DpcppComputeNonbonded::updateVdwTypesExcl() {
  ...
  memset(vdwTypes, 0, vdwTypesSize*sizeof(int)); // temporary to compare with reference dpcpp
  memset(exclIndexMaxDiff, 0, exclIndexMaxDiffSize*sizeof(int2)); // temporary to compare with reference dpcpp
  memset(atomIndex, 0, atomIndexSize*sizeof(int)); // temporary to compare with reference dpcpp
  ...
  nonbondedKernel.updateVdwTypesExcl(atomStorageSize, vdwTypes, exclIndexMaxDiff, atomIndex, myQ);
}

inline void DpcppComputeNonbonded::copyAtomsLoop(int first, int last, void *result, int paraNum, void *param) {
  DpcppComputeNonbonded* c = (DpcppComputeNonbonded *)param;
  c->copyAtomsSubset(first, last);
}

void DpcppComputeNonbonded::copyAtomsSubset(int first, int last) {
  for (int i=first;i <= last;++i) {
    PatchRecord &pr = patches[i];
    int numAtoms = pr.numAtoms;
    if (numAtoms > 0) {
      int start = pr.atomStart;
      const DpcppAtom *src = pr.patch->getDpcppAtomList();
      DpcppAtom *dst = atoms + start;
      memcpy(dst, src, sizeof(DpcppAtom)*numAtoms);
      // Fill the rest with the copy of the last atom
      int numAtomsAlign = ((numAtoms-1)/AVXSIZE+1)*AVXSIZE;
      DpcppAtom lastAtom = src[numAtoms-1];
...
void DpcppComputeNonbonded::copyGBISphase(int i) {
  if (CkMyPe() != patches[i].pe)
    NAMD_bug("DpcppComputeNonbonded::copyGBISphase called on wrong Pe");
  PatchRecord &pr = patches[i];
...
void DpcppComputeNonbonded::openBox(int i) {
  if (CkMyPe() != patches[i].pe)
    NAMD_bug("DpcppComputeNonbonded::openBox called on wrong Pe");
  SimParameters *simParams = Node::Object()->simParameters;
...
void DpcppComputeNonbonded::messageEnqueueWork() {
  if (masterPe != CkMyPe())
    NAMD_bug("DpcppComputeNonbonded::messageEnqueueWork() must be called from masterPe");
  WorkDistrib::messageEnqueueWork(this);
}

void DpcppComputeNonbonded::openBoxesOnPe() {
  if (rankPatches[CkMyRank()].size() == 0)
    NAMD_bug("DpcppComputeNonbonded::openBoxesOnPe, empty rank");
  for (int i=0;i < rankPatches[CkMyRank()].size();i++) {
...
void DpcppComputeNonbonded::doWork() {
  if (CkMyPe() != masterPe)
    NAMD_bug("DpcppComputeNonbonded::doWork() called on non masterPe");
...
void DpcppComputeNonbonded::launchWork() {
  if (CkMyPe() != masterPe)
    NAMD_bug("DpcppComputeNonbonded::launchWork() called on non masterPe");
  ...
  if (!simParams->GBISOn || gbisPhase == 1) {
    ...
    if (doStreaming) {
      ...
      if (patchReadyQueueLen != patches.size())
        NAMD_bug("DpcppComputeNonbonded::launchWork, invalid patchReadyQueueLen");
    }
  }
...
//    myQ.wait();
  ...
  if (!simParams->GBISOn || gbisPhase == 3) {
    if (!doStreaming) {
      copy_DtoH(d_forces, h_forces, atomStorageSize, myQ);
      if (doSlow) copy_DtoH(d_forcesSlow, h_forcesSlow, atomStorageSize, myQ);
myQ.wait();
    }
  }
  ...
  if ((!simParams->GBISOn || gbisPhase == 2) && (doEnergy || doVirial)) {
    ...
    nonbondedKernel.reduceVirialEnergy(tileListKernel,
      ...
      d_forces, d_forcesSlow, d_virialEnergy, myQ);
    copy_DtoH(d_virialEnergy, h_virialEnergy, 1, myQ);
myQ.wait();
  }
...
// GBIS Phase 1
//
void DpcppComputeNonbonded::doGBISphase1() {
#ifdef GBIS_CUDA
  dpcppDevice->setDeviceID(deviceID);

  if (atomsChanged) {
    GBISKernel.updateIntRad(atomStorageSize, intRad0H, intRadSH, myQ);
  }

  SimParameters *simParams = Node::Object()->simParameters;
  Lattice lattice = patches[0].patch->flags.lattice;

  float3 lata = float3(lattice.a().x, lattice.a().y, lattice.a().z);
  float3 latb = float3(lattice.b().x, lattice.b().y, lattice.b().z);
  float3 latc = float3(lattice.c().x, lattice.c().y, lattice.c().z);

  GBISKernel.GBISphase1(tileListKernel, atomStorageSize,
    lata, latb, latc,
    simParams->alpha_cutoff-simParams->fsMax, psiSumH, myQ);
#endif // GBIS_CUDA
}
...
// GBIS Phase 2
//
void DpcppComputeNonbonded::doGBISphase2() {
#ifdef GBIS_CUDA
  dpcppDevice->setDeviceID(deviceID);
  ...
  GBISKernel.updateBornRad(atomStorageSize, bornRadH, myQ);
...
// GBIS Phase 3
//
void DpcppComputeNonbonded::doGBISphase3() {
#ifdef GBIS_CUDA
  dpcppDevice->setDeviceID(deviceID);
  SimParameters *simParams = Node::Object()->simParameters;
  Lattice lattice = patches[0].patch->flags.lattice;
  ...
  if (doSlow) {
    GBISKernel.update_dHdrPrefix(atomStorageSize, dHdrPrefixH, myQ);

    GBISKernel.GBISphase3(tileListKernel, atomStorageSize,
      lata, latb, latc,
      simParams->alpha_cutoff-simParams->fsMax, d_forcesSlow, myQ);
  }
#endif // GBIS_CUDA
}
...
//
void DpcppComputeNonbonded::doForce() {
  dpcppDevice->setDeviceID(deviceID);
  ...
  if (doPairlist) {
    ...
    tileListKernel.buildTileLists(numTileLists, patches.size(), atomStorageSize,
      ...
      dpcppPatches, ( float4*)atoms, plcutoff2, maxShmemPerBlock, myQ);
    // Prepare tile list for atom-based refinement
    tileListKernel.prepareTileList(myQ);
  }
  ...
  nonbondedKernel.nonbondedForce(tileListKernel, atomStorageSize, doPairlist,
    ...
    ( float4*)atoms, cutoff2, d_forces, d_forcesSlow, h_forces, h_forcesSlow,
    myQ);

  if (doPairlist) {
    tileListKernel.finishTileList(myQ);
  }
}
...
//
int DpcppComputeNonbonded::calcNumTileLists() {
  int numTileLists = 0;
  for (int i=0;i < computes.size();i++) {
    int pi1 = computes[i].patchInd[0];
    int numAtoms1 = patches[pi1].numAtoms;
    int numTiles1 = (numAtoms1-1)/AVXSIZE+1;
...
//
void DpcppComputeNonbonded::finishReductions() {

  if (CkMyPe() != masterPe)
    NAMD_bug("DpcppComputeNonbonded::finishReductions() called on non masterPe");
  ...
  if (!doSkip) {
    ...
    if (doStreaming && (doVirial || doEnergy)) {
      ...
      if (!forceDoneEventRecord)
        NAMD_bug("DpcppComputeNonbonded::finishReductions, forceDoneEvent not being recorded");
      forceDoneEvent.wait();
      forceDoneEventRecord = false;
    }
    ...
    reduction->item(REDUCTION_EXCLUSION_CHECKSUM_DPCPP) += tileListKernel.getNumExcluded();
  }
...
//
void DpcppComputeNonbonded::finishPatch(int i) {
  if (CkMyPe() != patches[i].pe)
    NAMD_bug("DpcppComputeNonbonded::finishPatch called on wrong Pe");
  ...
  if (numAtoms > 0) {
    ...
    //   dpcppCheck(dpcppStreamSynchronize(myQ));
    //   NAMD_die("maxf!");
    // }
  }
...
//
void DpcppComputeNonbonded::finishSetOfPatchesOnPe(std::vector<int>& patchSet) {
  if (patchSet.size() == 0)
    NAMD_bug("DpcppComputeNonbonded::finishPatchesOnPe, empty rank");
  SimParameters *simParams = Node::Object()->simParameters;
...
//
void DpcppComputeNonbonded::finishPatchesOnPe() {
...
//
void DpcppComputeNonbonded::finishPatchOnPe(int i) {
  std::vector<int> v(1, i);
  finishSetOfPatchesOnPe(v);
}

void DpcppComputeNonbonded::finishPatches() {
  computeMgr->sendFinishPatchesOnPe(pes, this);
}

void DpcppComputeNonbonded::finishGBISPhase(int i) {
  if (CkMyPe() != patches[i].pe)
    NAMD_bug("DpcppComputeNonbonded::finishGBISPhase called on wrong Pe");
  PatchRecord &pr = patches[i];
  const CompAtomExt *aExt = pr.patch->getCompAtomExtInfo();
  int atomStart = pr.atomStart;
  if (gbisPhase == 1) {
...
void DpcppComputeNonbonded::finishTimers() {
  SimParameters *simParams = Node::Object()->simParameters;
}
...
//
void DpcppComputeNonbonded::reSortTileLists() {
  // Re-sort tile lists
  SimParameters *simParams = Node::Object()->simParameters;
  dpcppDevice->setDeviceID(deviceID);
  tileListKernel.reSortTileLists(simParams->GBISOn, myQ);
}

void DpcppComputeNonbonded::forceDoneCheck(void *arg, double walltime) {
  DpcppComputeNonbonded* c = (DpcppComputeNonbonded *)arg;

  if (CkMyPe() != c->masterPe)
    NAMD_bug("DpcppComputeNonbonded::forceDoneCheck called on non masterPe");
...
//        c->myQ.wait();
    ...
    if (!c->forceDoneEventRecord)
      NAMD_bug("DpcppComputeNonbonded::forceDoneCheck, forceDoneEvent not being recorded");
...
//      c->myQ.wait();
  ...
  if (c->checkCount >= 1000000) {
    char errmsg[256];
    sprintf(errmsg,"DpcppComputeNonbonded::forceDoneCheck polled %d times over %f s",
...
//
void DpcppComputeNonbonded::forceDoneSetCallback() {
  if (CkMyPe() != masterPe)
    NAMD_bug("DpcppComputeNonbonded::forceDoneSetCallback called on non masterPe");
  beforeForceCompute = CkWallTimer();
  dpcppDevice->setDeviceID(deviceID);
  if (!doStreaming || doVirial || doEnergy) {
    forceDoneEvent = myQ.single_task<class forceDoneEvent_empty_task>([=](){});
...
struct cr_sortop_distance {
  ...
  bool operator() (DpcppComputeNonbonded::ComputeRecord i,
      DpcppComputeNonbonded::ComputeRecord j) {
    Vector a = l.a();
    Vector b = l.b();
    Vector c = l.c();
...
struct cr_sortop_reverse_priority {
  ...
  const DpcppComputeNonbonded::PatchRecord *pr;
  cr_sortop_reverse_priority(cr_sortop_distance &sod,
       const DpcppComputeNonbonded::PatchRecord *patchrecs) : distop(sod), pr(patchrecs) { }
  bool pid_compare_priority(int2 pidi, int2 pidj) {
    const DpcppComputeNonbonded::PatchRecord &pri = pr[pidi.y()];
    const DpcppComputeNonbonded::PatchRecord &prj = pr[pidj.y()];
  ...
  bool operator() (DpcppComputeNonbonded::ComputeRecord j,
      DpcppComputeNonbonded::ComputeRecord i) {  // i and j reversed
    // Choose patch i (= patch with greater priority)
    int2 pidi = pid_compare_priority(int2(i.pid[0], i.patchInd[0]), int2(i.pid[1], i.patchInd[1])) ? int2(i.pid[0], i.patchInd[0]) : int2(i.pid[1], i.patchInd[1]);
    // Choose patch j
    int2 pidj = pid_compare_priority(int2(j.pid[0], j.patchInd[0]), int2(j.pid[1], j.patchInd[1])) ? int2(j.pid[0], j.patchInd[0]) : int2(j.pid[1], j.patchInd[1]);
...
//
void DpcppComputeNonbonded::updateComputes() {
  dpcppDevice->setDeviceID(deviceID);
  ...
  DpcppComputeRecord* dpcppComputes = new DpcppComputeRecord[computes.size()];

  for (int i=0;i < computes.size();i++) {
    dpcppComputes[i].patchInd.x() = computes[i].patchInd[0];
    dpcppComputes[i].patchInd.y() = computes[i].patchInd[1];
    dpcppComputes[i].offsetXYZ.x() = computes[i].offset.x;
    dpcppComputes[i].offsetXYZ.y() = computes[i].offset.y;
    dpcppComputes[i].offsetXYZ.z() = computes[i].offset.z;
  }

  tileListKernel.updateComputes(computes.size(), dpcppComputes, myQ);
  myQ.wait();

  delete [] dpcppComputes;
}
...
//
void DpcppComputeNonbonded::buildExclusions() {
  dpcppDevice->setDeviceID(deviceID);
...
#define SET_EXCL(EXCL,BASE,DIFF) \
  ...
  for ( int i=0; i<unique_lists.size(); ++i ) {
    ...
    if ( unique_lists[i][2] != (int32)base ) {
      NAMD_bug("DpcppComputeNonbonded::build_exclusions base != stored");
    }
