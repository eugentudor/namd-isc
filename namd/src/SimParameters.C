...
#ifdef NAMD_DPCPP
    // update LJTable for GPU, needs CPU update first
    DpcppComputeMgr::getDpcppComputeMgr()->update();
#endif
    return;
  }
...
#ifdef NAMD_DPCPP
   if (loweAndersenOn) {
       NAMD_die("Lowe-Andersen dynamics not compatible with DPC++ at this time");
   }
#endif
   // END LA
...
#endif
#ifdef NAMD_DPCPP
       NAMD_die("alchWCA is not currently available with DPC++");
#endif
     }
...
//     }
     ...
     bool one_device_per_node = dpcppDevice->one_device_per_node();  // only checks node 0
     if ( ! opts.defined("PMEOffload") ) {
       PMEOffload = ( (PMEInterpOrder > 4) && one_device_per_node );
       if ( PMEOffload ) iout << iINFO << "Enabling PMEOffload because PMEInterpOrder > 4.\n" << endi;
     } else if ( PMEOffload && ! one_device_per_node ) {
...
if ( openatomOn ) 
{
  ...
  iout << iINFO << "OPENATOM PDB FILE:     " << openatomPDB << "\n";
   ...
   if ( ! usePairlists ) {
     usePairlists = 1;
     iout << iINFO << "DPC++ ACCELERATION REQUIRES PAIRLISTS\n";
   }
#endif
