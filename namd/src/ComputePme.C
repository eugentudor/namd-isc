...
public:
...
#if defined(NAMD_CUDA) || defined(NAMD_DPCPP)
  float *a_data_host;
  float *a_data_dev;
  float *f_data_host;
  float *f_data_dev;
  ...
  void submit_charges(Lattice &lattice, int sequence);
  struct submit_charges_args {
    ComputePmeMgr *mgr; Lattice *lattice; int sequence;
  };
  static std::deque<submit_charges_args> submit_charges_deque;
...
#if defined(NAMD_CUDA) || defined(NAMD_DPCPP)
CmiNodeLock ComputePmeMgr::smp_lock;
std::deque<ComputePmeMgr::submit_charges_args> ComputePmeMgr::submit_charges_deque;
bool ComputePmeMgr::device_busy;
#endif
...
#elif defined(NAMD_DPCPP)
  sycl::event end_charge_memset;
  sycl::event end_all_pme_kernels;
  sycl::event end_potential_memcpy;
#endif
};
...
#else
  NAMD_bug("NodePmeMgr::recvUngrid called in non-CUDA/DPC++ build.");
#endif
}
...
#elif defined(NAMD_DPCPP)
  // Create queue from global queue
  const sycl::queue& default_queue = getDefaultQ();
  stream = sycl::queue(default_queue.get_context(), default_queue.get_device(),
      dpcppDevice->getExceptionHandler(), sycl::property::queue::in_order());

  // Create a SYCL event for querying charge kernel completion
  end_charges = sycl::event();

  // Register user events
#define DPCPP_EVENT_ID_PME_CHARGES 80
#define DPCPP_EVENT_ID_PME_FORCES 81
#define DPCPP_EVENT_ID_PME_TICK 82
#define DPCPP_EVENT_ID_PME_COPY 83
#define DPCPP_EVENT_ID_PME_KERNEL 84
  if (0 == CkMyPe()) {
    traceRegisterUserEvent("DPC++ PME charges", DPCPP_EVENT_ID_PME_CHARGES);
    traceRegisterUserEvent("DPC++ PME forces",  DPCPP_EVENT_ID_PME_FORCES);
    traceRegisterUserEvent("DPC++ PME tick",    DPCPP_EVENT_ID_PME_TICK);
    traceRegisterUserEvent("DPC++ PME memcpy",  DPCPP_EVENT_ID_PME_COPY);
    traceRegisterUserEvent("DPC++ PME kernel",  DPCPP_EVENT_ID_PME_KERNEL);
  }
#endif // NAMD_CUDA, NAMD_DPCPP
...
#elif defined(NAMD_DPCPP)
  if (offload && !dpcppDevice->one_device_per_node()) {
    NAMD_die("PME offload requires exactly one DPC++ device per process.  Use \"PMEOffload no\".");
  }
  if (offload) {
    int dev = dpct::dev_mgr::instance().current_device_id();
    if (dev != dpcppDevice->getDeviceID()) {
      NAMD_bug("ComputePmeMgr::initialize dev != dpcppDevice->getDeviceID()");
    }
  }
  // Don't check compute capability with DPC++
#endif // NAMD_CUDA, NAMD_DPCPP
...
#elif defined(NAMD_DPCPP)
        if (CkMyPe() == dpcppDevice->getMasterPe() || !offload)
#endif
...
#elif defined(NAMD_DPCPP)
#define count_limit 1000000
#define DPCPP_POLL(FN,ARG) CcdCallFnAfter(FN,ARG,0.1)
#define EVENT_STRIDE 10
...
void dpcpp_check_pme_forces(void *arg, double walltime) try {
  ComputePmeMgr *argp = (ComputePmeMgr *) arg;

  while ( 1 ) { // process multiple events per call
    auto event_status = argp->end_forces[argp->forces_done_count/EVENT_STRIDE]
    ...
    if (event_status == sycl::info::event_command_status::complete) {
      ...
      if (argp->forces_done_count == argp->forces_count) { // last event
        traceUserBracketEvent(DPCPP_EVENT_ID_PME_FORCES, argp->forces_time, walltime);
        argp->forces_time = walltime - argp->forces_time;
        return;
      } else { // more events
        continue; // check next event
  ...
  CcdCallBacksReset(0, walltime);  // fix Charm++
  DPCPP_POLL(dpcpp_check_pme_forces, arg);
...
void ComputePmeMgr::ungridCalc(void) {
 ...
 if ( offload ) {
  ...
  for ( int i=0; i<pcsz; ++i ) {
    ...
    afn_host[3*i  ] = a_data_dev + 7 * pmeComputes[i]->atoms_offset;
...
#elif defined(NAMD_DPCPP)
#define CALL(ORDER) \
   if (myGrid.order == ORDER) \
     dpcpp_pme_forces<ORDER>(bspline_coeffs_dev, v_arr_dev, afn_dev+3*i, \
         dimy, maxn, myGrid.K1, myGrid.K2, myGrid.K3, stream)

  if (offload) {
    dpct::dev_mgr::instance().select_device(dpcppDevice->getDeviceID());

    if (this == masterPmeMgr) {
      double before = CmiWallTimer();
      nodePmeMgr->end_potential_memcpy = stream.memcpy(
          v_data_dev, q_data_host, q_data_size);
      nodePmeMgr->end_potential_memcpy.wait();
      traceUserBracketEvent(DPCPP_EVENT_ID_PME_COPY,before,CmiWallTimer());
    ...
    if (!end_forces) {
      int n = (pmeComputes.size()-1)/EVENT_STRIDE+1;
      end_forces = new sycl::event[n];
    }

    const int pcsz = pmeComputes.size();
    if (!afn_host) {
      afn_host = sycl::malloc_host<float *>(3 * pcsz, stream);
      afn_dev = sycl::malloc_device<float *>(3 * pcsz, stream);
    }
    int totn = 0;
    for (int i = 0; i < pcsz; ++i) {
      int n = pmeComputes[i]->numGridAtoms[0];
    ...
    if (totn > f_data_mgr_alloc) {
      if (f_data_mgr_alloc) {
        CkPrintf("Expanding CUDA forces allocation because %d > %d\n", totn, f_data_mgr_alloc);
        sycl::free(f_data_mgr_dev, stream);
        sycl::free(f_data_mgr_host, stream);
      }
      f_data_mgr_alloc = 1.2 * (totn + 100);
      f_data_mgr_dev = sycl::malloc_device<float>(3 * f_data_mgr_alloc, stream);
      f_data_mgr_host = sycl::malloc_host<float>(3 * f_data_mgr_alloc, stream);
      stream.memset(f_data_mgr_dev, 0, 3 * f_data_mgr_alloc * sizeof(float));
    }
    float *f_dev = f_data_mgr_dev;
    float *f_host = f_data_mgr_host;
    for (int i = 0; i < pcsz; ++i) {
    ...
    double before = CmiWallTimer();
    stream.memcpy(afn_dev, afn_host, 3 * pcsz * sizeof(float *));
    traceUserBracketEvent(DPCPP_EVENT_ID_PME_COPY,before,CmiWallTimer());
/* DPCT_ORIG   cudaStreamWaitEvent(streams[stream],
 * nodePmeMgr->end_potential_memcpy, 0);*/
    // TODO: No equivalent in SYCL for cudaStreamWaitEvent
    nodePmeMgr->end_potential_memcpy.wait();
    traceUserEvent(DPCPP_EVENT_ID_PME_TICK);
    ...
    for (int i = 0; i < pcsz; ++i) {
      if (i % EVENT_STRIDE == 0) {
        ...
        /*
        AC_TAG("pme_forces");
        AC_COMP_DEVICE(stream, "pre_pme_forces", bspline_coeffs_dev, myGrid.order*myGrid.order);
        AC_COMP_DEVICE(stream, "pre_pme_forces", v_data_dev, q_data_size/sizeof(float));
        AC_COMP_DEVICE(stream, "pre_pme_forces", a_data_dev, 7*atoms_alloc);
        AC_COMP_DEVICE(stream, "pre_pme_forces", f_data_mgr_dev, 3*f_data_mgr_alloc);
        */
        ...
        /*
        AC_COMP_DEVICE(stream, "post_pme_forces", bspline_coeffs_dev, myGrid.order*myGrid.order);
        AC_COMP_DEVICE(stream, "post_pme_forces", v_data_dev, q_data_size/sizeof(float));
        AC_COMP_DEVICE(stream, "post_pme_forces", a_data_dev, 7*atoms_alloc);
        AC_COMP_DEVICE(stream, "post_pme_forces", f_data_mgr_dev, 3*f_data_mgr_alloc);
        */

        traceUserBracketEvent(DPCPP_EVENT_ID_PME_KERNEL, before, CmiWallTimer());
        before = CmiWallTimer();
        end_forces[i/EVENT_STRIDE] = stream.memcpy(pmeComputes[i]->f_data_host,
            pmeComputes[i]->f_data_dev, 3 * subtotn * sizeof(float));
        traceUserBracketEvent(DPCPP_EVENT_ID_PME_COPY, before, CmiWallTimer());
        traceUserEvent(DPCPP_EVENT_ID_PME_TICK);
      }
    }
...
void ComputePmeMgr::pollForcesReady() {
  ...
  DPCPP_POLL(dpcpp_check_pme_forces, this);
#else
  NAMD_bug("ComputePmeMgr::pollForcesReady() called in non-CUDA or non-DPC++ build.");
#endif
}
...
#if defined(NAMD_CUDA) || defined(NAMD_DPCPP)
 if ( offload ) {
  myMgr->atoms_count += patch->getNumAtoms();
 }
#endif
}
...
void ComputePmeMgr::initialize_computes() {
 ...
 for ( int i=0; i<CkMyNodeSize(); ++i, ++pe ) {
    ...
    if ( master_pe < 1 && pe != got_master_pe ) master_pe = pe;
    if ( master_pe == got_master_pe ) master_pe = pe;
 ...
 if ( offload ) {
  CmiLock(smp_lock);
  deviceFirst = ! masterPmeMgr->chargeGridSubmittedCount++;
 }
...
#elif defined(NAMD_DPCPP)
  q_arr_dev = sycl::malloc_device<float *>(fsize, stream);
  v_arr_dev = sycl::malloc_device<float *>(fsize, stream);
#endif
...
#elif defined(NAMD_DPCPP)
  q_data_host = (float *)sycl::malloc_host(q_data_size + ffz_size, stream);
  q_data_dev = (float *)sycl::malloc_device(q_data_size + ffz_size, stream);
  v_data_dev = (float *)sycl::malloc_device(q_data_size, stream);
  nodePmeMgr->end_charge_memset = stream.memset(
      q_data_dev, 0, q_data_size + ffz_size);
  nodePmeMgr->end_charge_memset.wait(); // for first time
#endif
...
#elif defined(NAMD_DPCPP)
  stream.memcpy(q_arr_dev, q_arr_dev_host, fsize * sizeof(float *)).wait();
  stream.memcpy(v_arr_dev, v_arr_dev_host, fsize * sizeof(float *)).wait();
#endif
...
#elif defined(NAMD_DPCPP)
  dpcpp_init_bspline_coeffs(&bspline_coeffs_dev, &bspline_dcoeffs_dev, myGrid.order, stream);
#endif
...
#if defined(NAMD_CUDA) || defined(NAMD_DPCPP)
   if ( offload ) {
    if ( myMgr->atoms_alloc == 0 ) {  // first call
      int na = myMgr->atoms_alloc = 1.2 * (myMgr->atoms_count + 1000);
#ifdef NAMD_CUDA
...
#elif defined(NAMD_DPCPP)
      try {
        (myMgr->a_data_host) = sycl::malloc_host<float>(7 * na, myMgr->stream);
        (myMgr->a_data_dev) = sycl::malloc_device<float>(7 * na, myMgr->stream);
      } catch (sycl::exception const &exc) {
        std::cerr << exc.what() << "Exception caught at file:" << __FILE__
                  << ", line:" << __LINE__ << std::endl;
        std::exit(1);
...
#elif defined(NAMD_DPCPP)
      try {
        CkPrintf("PE %d expanding DPC++ PME atoms allocation because %d > %d\n",
            CkMyPe(), myMgr->atoms_count, myMgr->atoms_alloc);
        (myMgr->a_data_host) = sycl::malloc_host<float>(7 * na, myMgr->stream);
        memcpy(myMgr->a_data_host, a_data_host_old, 7*atoms_offset*sizeof(float));
        sycl::free((void *)a_data_host_old, myMgr->stream);
        sycl::free(myMgr->a_data_dev, myMgr->stream);
        (myMgr->a_data_dev) = sycl::malloc_device<float>(7 * na, myMgr->stream);
      } catch (sycl::exception const &exc) {
        std::cerr << exc.what() << "Exception caught at file:" << __FILE__
                  << ", line:" << __LINE__ << std::endl;
    ...
    float *a_data_host = myMgr->a_data_host + 7 * atoms_offset;
    data_ptr = localGridData[g];
...
// cudaDeviceSynchronize();  // XXXX
  ...
  if ( offload ) {
    int master_pe;
#ifdef NAMD_CUDA
    master_pe = deviceCUDA->getMasterPe();
#elif defined(NAMD_DPCPP)
    master_pe = dpcppDevice->getMasterPe();
#endif
    ComputePmeMgr::submit_charges_args args;
    args.mgr = myMgr;
    args.lattice = &lattice;
    args.sequence = sequence();
    CmiLock(ComputePmeMgr::smp_lock);
    if ( ComputePmeMgr::device_busy ) {
      ComputePmeMgr::submit_charges_deque.push_back(args);
    } else if ( CkMyPe() == master_pe ) {
      // avoid adding work to nonbonded data preparation pe
      args.mgr->submit_charges(*args.lattice, args.sequence);
    } else {
      ComputePmeMgr::device_busy = true;
      while ( 1 ) {
        CmiUnlock(ComputePmeMgr::smp_lock);
        args.mgr->submit_charges(*args.lattice, args.sequence);
        CmiLock(ComputePmeMgr::smp_lock);
        if ( ComputePmeMgr::submit_charges_deque.size() ) {
          args = ComputePmeMgr::submit_charges_deque.front();
          ComputePmeMgr::submit_charges_deque.pop_front();
        } else {
          ComputePmeMgr::device_busy = false;
          break;
        }
      }
    }
...
#ifdef NAMD_CUDA
void ComputePmeMgr::submit_charges(Lattice &lattice, int sequence) {

    int n = atoms_count;
    //CkPrintf("pe %d atoms_count %d\n", CkMyPe(), atoms_count);
    atoms_count = 0;
...
#elif defined(NAMD_DPCPP)
void ComputePmeMgr::submit_charges(Lattice &lattice, int sequence) try {
#define CALL(ORDER) \
  if (myGrid.order == ORDER) \
    dpcpp_pme_charges<ORDER>(bspline_coeffs_dev, q_arr_dev, ffz_dev, \
        ffz_dev + fsize, a_data_dev, n, myGrid.K1, myGrid.K2, myGrid.K3, \
        stream) \
    ...
    const double before = CmiWallTimer();
    stream.memcpy(a_data_dev, a_data_host, 7 * n * sizeof(float));
    const double after = CmiWallTimer();

    // TODO: Make the stream wait for the event instead of the host
    //cudaStreamWaitEvent(streams[stream], nodePmeMgr->end_charge_memset, 0);
    nodePmeMgr->end_charge_memset.wait();

    /*
    AC_TAG("pme_charges");
    AC_COMP_DEVICE(stream, "pre_pme_charges", bspline_coeffs_dev, myGrid.order*myGrid.order);
    AC_COMP_DEVICE(stream, "pre_pme_charges", q_data_dev, q_data_size/sizeof(float));
    AC_COMP_DEVICE(stream, "pre_pme_charges", ffz_dev, ffz_size/sizeof(int));
    AC_COMP_DEVICE(stream, "pre_pme_charges", a_data_dev, 7*n);
    */
...
#undef CALL
    /*
    AC_COMP_DEVICE(stream, "post_pme_charges", bspline_coeffs_dev, myGrid.order*myGrid.order);
    AC_COMP_DEVICE(stream, "post_pme_charges", q_data_dev, q_data_size/sizeof(float));
    AC_COMP_DEVICE(stream, "post_pme_charges", ffz_dev, ffz_size/sizeof(int));
    AC_COMP_DEVICE(stream, "post_pme_charges", a_data_dev, 7*n);
    */
    ...
    masterPmeMgr->charges_time = before;
    traceUserBracketEvent(DPCPP_EVENT_ID_PME_COPY,before,after);
    traceUserBracketEvent(DPCPP_EVENT_ID_PME_KERNEL,after,after2);
} catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
  std::exit(1);
...
void dpcpp_check_pme_charges(void *arg, double walltime) try {
  ComputePmeMgr *argp = (ComputePmeMgr *)arg;

  auto event_status = argp->end_charges
    .get_info<sycl::info::event::command_execution_status>();
  if (event_status == sycl::info::event_command_status::complete) {
    traceUserBracketEvent(DPCPP_EVENT_ID_PME_CHARGES,argp->charges_time,walltime);
    argp->charges_time = walltime - argp->charges_time;
    argp->sendChargeGridReady();
    argp->check_charges_count = 0;
  } else if ( ++(argp->check_charges_count) >= count_limit ) {
    char errmsg[256];
    sprintf(errmsg,"dpcpp_check_pme_charges polled %d times over %f s on seq %d",
            argp->check_charges_count, walltime - argp->charges_time,
            argp->saved_sequence);
    dpcppDie(errmsg,0);
  } else {
    CcdCallBacksReset(0, walltime);  // fix Charm++
    DPCPP_POLL(dpcpp_check_pme_charges, arg);
  }
} catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
...
void ComputePmeMgr::chargeGridSubmitted(Lattice &lattice, int sequence) {
  ...
  if ( --(masterPmeMgr->chargeGridSubmittedCount) == 0 ) {
    ...
    //cudaStreamWaitEvent(streams[stream], nodePmeMgr->end_all_pme_kernels, 0);
    stream.wait();
    masterPmeMgr->end_charges = stream.memcpy(q_data_host, q_data_dev, q_data_size + ffz_size);
    traceUserBracketEvent(DPCPP_EVENT_ID_PME_COPY,before,CmiWallTimer());
    nodePmeMgr->end_charge_memset = stream.memset(q_data_dev, 0,
        q_data_size + ffz_size); // for next time

    SimParameters *simParams = Node::Object()->simParameters;
    if ( ! simParams->useCUDA2 ) { // TODO: Change useCUDA2?
      CProxy_ComputeMgr cm(CkpvAccess(BOCclass_group).computeMgr);
#ifdef NAMD_CUDA
      cm[deviceCUDA->getMasterPe()].recvYieldDevice(-1);
#elif defined(NAMD_DPCPP)
      cm[dpcppDevice->getMasterPe()].recvYieldDevice(-1);
#endif
    }
...
#elif defined(NAMD_DPCPP)
  CcdCallBacksReset(0, CmiWallTimer());
  DPCPP_POLL(dpcpp_check_pme_charges, this);
#else
  NAMD_bug("ComputePmeMgr::pollChargeGridReady() called in non-CUDA or non-DPC++ build.");
#endif
}
...
void NodePmeMgr::sendPencilsHelper(int iter) {
  ...
  obj->sendPencilsPart(iter, iter, *obj->sendDataHelper_lattice,
      obj->sendDataHelper_sequence, obj->sendDataHelper_sourcepe);
#else
  NAMD_bug("NodePmeMgr::sendPencilsHelper called in non-CUDA or non-DPC++ build");
#endif
}
...
#if defined(NAMD_CUDA) || defined(NAMD_DPCPP)
  ComputePmeMgr *obj = masterPmeMgr;
  obj->sendDataPart(iter, iter, *obj->sendDataHelper_lattice,
      obj->sendDataHelper_sequence, obj->sendDataHelper_sourcepe, obj->sendDataHelper_errors);
#else
  NAMD_bug("NodePmeMgr::sendDataHelper called in non-CUDA or non-DPC++ build");
#endif
}
